    ...
    public function attachForm()
    {
        return Forms\Components\Group::make()
                ->schema([
                    ...
                    //     return $horse;
                    // })
                    ->disableOptionWhen(function (RelationManager $livewire,string $value, Get $get, string $operation, ?string $state): bool {
                        $available =  Horse::whereDoesntHave('slots', function($q) use($livewire) {
                                    $q->where('slots.id', $livewire->getOwnerRecord()->id);
                                })->pluck('id')->toArray();
                                
                        if($operation == 'edit'){
                            return !in_array($value, $available) && $value != $state;
                        }
                        return !in_array($value, $available);
                    })
                    ->rules([
                        fn (RelationManager $livewire,Get $get, string $operation, ?string $state): Closure => function (string $attribute, $value, Closure $fail) use($get, $operation,$state, $livewire) {
                            $available =  Horse::whereDoesntHave('slots', function($q) use($livewire) {
                                $q->where('slots.id', $livewire->getOwnerRecord()->id);
                            })->pluck('id')->toArray();
                            if($operation == 'edit'){
                                if(!in_array($value, $available) && $value != $state){
                ...
                Forms\Components\Select::make('rider_id')
                    ...
                    //     $record_id = $livewire->getOwnerRecord()->id ;

                    //     $rider = DB::select("SELECT a.id, a.name
                    //     FROM riders a
                    //     WHERE a.id NOT IN (
                    //     SELECT CASE 
                    ...
                    //     ) OR a.id = ?", [$record_id, $state]);
                        
                    //     $rider = collect($rider)->pluck('name', 'id');
                    //     return $rider;
                    // })
                    ->disableOptionWhen(function (RelationManager $livewire,string $value, Get $get, string $operation, ?string $state): bool {
